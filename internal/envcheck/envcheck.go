...
// ставить libgl1 бесполезно); нехватка ЖЁСТКОЙ зависимости перевешивает
// «определить не удалось» — без libGL процесс не стартует, что бы мы ни
// узнали об остальном; «определить не удалось» перевешивает «скорее всего,
// запустится» — предположение о запуске не имеет права стоять выше
// признанного незнания. Отсутствие графической сессии на выбор итога не
// влияет: по SSH её нет закономерно.
func verdict(r Result) string {
	...
	// Сбор «определить не удалось» стоит ВЫШЕ ветки «скорее всего,
	// запустится»: иначе непустой MissingDlopen перехватывает незнание про
	// библиотеку C, и программа обещает запуск, не сумев определить то, от
	// чего запуск зависит (место № 3 задания A1, признак 3 «порядок
	// ветвей»). Ветки выше — musl, старая glibc, нехватка жёсткой
	// зависимости — остаются выше сбора: каждая из них не предположение, а
	// установленный факт.
	//
	...
	if r.Graph.Known && len(r.Graph.MissingDlopen) > 0 {
		// Сюда доходим, только когда про библиотеку C известно всё: иначе
		// «скорее всего» было бы сказано поверх незнания. Сами имена
		// ненайденных библиотек человек в любом случае видит строкой
		// «Библиотеки графики: главные на месте, остальных не видно: …» —
		// перенос сбора выше их не прячет.

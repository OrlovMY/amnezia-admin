...
// ПЕРЕЧЕНЬ ПУСТ С A4в. Единственное исключение —
// "writeConfigFile/filepath.Abs" (abs, _ := filepath.Abs(fileName)) — исчезло
// не потому, что его вычеркнули, а потому, что исчезло само место: A4в убрал
// из writeConfigFile вычисление абсолютного пути вовсе, каталог теперь
// абсолютен по построению (core.UserConfigsDir). Долг погашен, а не списан.
...
// различается.
//
// ЧЕМ МЕРИТСЯ «РАЗБОР ЖИВ» (изменено в A4в). Раньше мерилось числом НАХОДОК:
// в cmd/gui было ровно одно место отброшенной ошибки, и ноль находок означал
// сломанный разбор. A4в это место убрал — оно было единственным, — и мера
// перестала годиться: теперь ноль находок это ЗАКОННОЕ состояние, и прежний
// текст требовал бы держать долг ради зелени сторожа. Мера заменена на ту,
// что не зависит от долга: разбор обязан видеть в cmd/gui объявления функций.
// Ноль функций — это переезд пакета или сломанный парсер, а не «долгов нет».
func TestGuiDiscardGuardStillSeesSomething(t *testing.T) {
	funcs := 0
	for _, path := range guiGoFiles(t) {
		funcs += countFuncDecls(t, path)
	}
	if funcs == 0 {
		t.Fatalf("сторож Г5 ПЕРЕСТАЛ ЧТО-ЛИБО ПРОВЕРЯТЬ: в %s разбор не увидел НИ ОДНОГО "+
			"объявления функции — пакет переехал или разбор сломан", guiPkgFiles)
	}

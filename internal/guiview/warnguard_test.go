...
// вызов" НЕ отличает пять от одного: предупреждение, подключённое к удалению
...
// вызов обязан быть в пяти названных обработчиках, — а не описываем способы
...
// присутствует в КАЖДОМ из пяти обработчиков необратимых операций, и вход
...
func TestGUICallsWarningInEveryHandler(t *testing.T) {
	...
	// Пункт 3: ни одного из пяти обработчиков не нашли — структура файла
	...
	if seen == 0 {
		t.Fatalf("сторож Г5 ПЕРЕСТАЛ ЧТО-ЛИБО ПРОВЕРЯТЬ: в %s не найден НИ ОДИН из пяти обработчиков %v — "+

...
func listStatsNote(statsFailed bool, absent int) string {
	...
	case absent > 0:
		return fmt.Sprintf("Нет в статистике сервера: %d (запись в clientsTable есть, а работающий wg "+
			"этих клиентов не держит — подключиться они сейчас не могут); их активность и трафик неизвестны («?»).", absent)
	}
	return ""
}

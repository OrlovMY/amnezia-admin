...
func hoverAt(t *testing.T, u *ui, at fyne.Position) {
	...
	// walkVisible, а не walkObjects: боевой драйвер ищет получателя движения
	// только среди видимого, внутрь скрытого не спускаясь.
...
// обычно снимает захват (clientTable.MouseUp), но оно может НЕ ДОЙТИ до
// таблицы вовсе: alt-tab с зажатой кнопкой, потеря окном фокуса, перехват
// ввода другой программой. Тогда граница остаётся захваченной. (Уехавший на
// кнопку курсор сюда не подходит: порог протаскивания — две точки, и его
// конец драйвер сопровождает DragEnd сам.) Следующий обычный клик по подписи

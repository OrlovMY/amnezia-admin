...
// LayoutSuspectIgnoringLineBreaks — тот же признак, но ПЕРЕНОС СТРОКИ им не
// считается.
//
// ЗАЧЕМ ОТДЕЛЬНАЯ ФУНКЦИЯ (ревью UX-01). Поле админского ключа —
// многострочное, а ключ vpn://… длинный: его регулярно копируют из
// мессенджера или почты уже разбитым на строки. Перенос строки — это символ
// вне печатаемого ASCII, и общий признак назвал бы его «не английской
// раскладкой»: человек смотрел бы на латиницу и читал, что у него не та
// раскладка. Уверенный неверный ответ там, где причина другая, — ровно то,
// что запрещает CLAUDE.md.
//
// Одно место истины сохранено: набор допустимых символов по-прежнему один
// (pinCharClass), здесь только выброшены \r и \n перед той же проверкой.
// Ключ с переносами всё равно не подойдёт, но скажет об этом тот, кто знает
// причину, а не подсказка про раскладку.
func LayoutSuspectIgnoringLineBreaks(s string) bool {
	return NonEnglishLayoutSuspect(strings.NewReplacer("\r", "", "\n", "").Replace(s))
}

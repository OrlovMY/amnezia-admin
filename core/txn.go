...
func maskPEM(s string) string {
	...
	// Обрыв С КОНЦА: BEGIN есть, END не дошёл.
	for _, loc := range rePEMHeader.FindAllStringIndex(s, -1) {
		...
		if strings.Contains(strings.ToUpper(rest), "-----END") {
			continue
		}
		// M1 (ревью SEC-01): пометку об обрезке ставим только если что-то
		// действительно обрезано. BEGIN в самом конце текста ничего за собой
		// не имеет, и пометка утверждала бы факт, которого не было, — тот же
		// класс «незнание выдано за знание», только наизнанку.
	...
	// Обрыв С НАЧАЛА: END есть, а BEGIN выше нет (ревью SEC-01, V2). Это те
	// же условия, ради которых заведена ветка обрыва с конца: кольцевой
	// буфер, tail, обрезка чужой утилитой — только сообщение обрезано
	// спереди, и тело ключа выходит открытым перед строкой END.
	if loc := rePEMFooter.FindStringIndex(s); loc != nil {
		head := s[:loc[0]]
		if !strings.Contains(strings.ToUpper(head), "-----BEGIN") &&
			!strings.HasSuffix(strings.TrimSpace(head), hiddenPlaceholder) {
			tail := s[loc[1]:]
			note := ""
			if strings.TrimSpace(head) != "" {
				note = truncatedNoteHead
			}
			return hiddenPlaceholder + note + s[loc[0]:loc[1]] + tail
		}
	}
	return s
}

...
//
// ЧЕГО ЭТА ПРОВЕРКА НЕ ДЕЛАЕТ (A8, п.2; П-НЕЗНАНИЕ применяется и к отчётам
// проверок — «проверено» не должно означать «проверено частично»):
// в РАНТАЙМЕ сверяется ТОЛЬКО набор публичных ключей. AllowedIPs и
// PresharedKey активных peer'ов с тем, что записано в wg0.conf, НЕ
// сверяются, хотя `wg show wg0 dump` их и отдаёт (поля 1 и 3).
// Сознательное решение A8, а не упущение: расхождение здесь означало бы
// откат уже удавшейся операции, а нормализация AllowedIPs рантаймом против
// текста wg0.conf (порядок, форма префикса, IPv6) на боевом сервере Amnezia
// нами не измерена — ложный откат на живом VPN хуже неполной проверки.
// Поэтому граница проверки названа вслух здесь и в тексте, который видит
// человек (см. restore), а не выдана за полную.
//
// Следствие: файлы гарантированы байт в байт, поэтому расхождение AllowedIPs
...
func (s *Session) restore(c *Container, p *Plan, wgChanged bool, cause error) error {
	...
	if verifyErr == nil {
		...
		// оно означает «проверено частично», выданное за полное.
		const scope = " (сверены: содержимое обоих файлов байт в байт и набор активных peer'ов; AllowedIPs и PSK в рантайме НЕ сверялись)"
		// A8 п.3: «вернул прежний файл» и «создал новый, прежнего не было» —
		// разные вещи, и во второй формулировке слова «восстановлено» нет.
		// Файл clientsTable создаёт шаг применения (applySteps пишет его
		// всегда); отката «не было файла» у нас нет — мы оставляем его
		...
		if !p.tblExisted {
			return fmt.Errorf("операция отменена, откат выполнен и проверен%s. ВНИМАНИЕ: clientsTable до операции НЕ СУЩЕСТВОВАЛА — отсутствие файла вернуть нечем, файл оставлен ПУСТЫМ; для этой утилиты пустая таблица и отсутствующая равнозначны (список пользователей пуст и там, и там). Исходная причина: %v", scope, cause)
		}
		return fmt.Errorf("операция отменена, состояние восстановлено и проверено%s: %v", scope, cause)
	}

...
func (s *Session) verify(c *Container, p *Plan, checkPeers bool) error {
	...
	if err != nil {
		return fmt.Errorf("проверка активных подключений: %w", err)
	}
	want := peerPubKeysFromBytes(p.wgAfter)
	if len(stats) != len(want) {
		return fmt.Errorf("проверка не пройдена: набор активных подключений на сервере не совпадает с ожидаемым")
	}
	for pk := range want {
		if _, ok := stats[pk]; !ok {

...
//  7. МАСКИРОВКА ОБРЫВАЕТ ТЕКСТ ПОСЛЕ PEM-ЗАГОЛОВКА БЕЗ ЗАВЕРШЕНИЯ —
//     остаток сообщения теряется. Потеря НЕ молчаливая: на месте обрыва
//     ставится truncatedNote, чтобы человек отличил «сервер замолчал» от
//     «мы обрезали».
//
...
// looksLikeSecret — похоже ли незакавыченное значение на секрет.
func looksLikeSecret(v string) bool {
	if len(v) >= minUnquotedSecretLen && reSecretAlphabet.MatchString(v) {
		return true
	}
	return reLongSecretRun.MatchString(v)
}
...
func maskFreeText(s string) string {
	...
	return reFreeTextSecret.ReplaceAllStringFunc(s, func(m string) string {
		...
		if !reQuotedSep.MatchString(sep) && !looksLikeSecret(val) {
			return m
		}

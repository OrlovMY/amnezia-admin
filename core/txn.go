...
// маскировка ПО ИЗВЕСТНЫМ ИМЕНАМ. Не ловится: (1) секрет, напечатанный
// сервером БЕЗ ИМЕНИ рядом (голая строка ключа в сообщении об ошибке) —
// это не ловится ничем, кроме наблюдения; (2) секрет под именем, которого
// нет в secretKeyNames — ловится добавлением имени, то есть ПОСЛЕ того, как
// утечка уже случилась хотя бы раз.
//
// reFreeTextSecret — «<имя секрета><разделитель><значение>» в любом месте
// строки, в любом регистре. Разделитель — ":" или "=", с необязательными
// кавычками и пробелами (покрывает и "PresharedKey = X", и `"psk": "X"`, и
// "privatekey=X"). Значение — до первого пробела, кавычки, запятой,
// точки с запятой или апострофа: закрывающая кавычка в значение не входит и
// потому остаётся в тексте, структура строки не рвётся.
var reFreeTextSecret = regexp.MustCompile(`(?i)\b(` + secretNamesAlt + `)("?\s*[:=]\s*"?)([^\s",;']+)`)

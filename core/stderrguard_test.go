...
//
// Сторож НЕ ВИДИТ захвата через промежуточную переменную с другим именем, НЕ
// РАЗБИРАЕТ ПОТОК ДАННЫХ и НЕ ОТЛИЧИТ stderr, прошедший через три
// присваивания, от любой другой строки. Он ловит один конкретный вид:
// вызов .String() на переменной, объявленной в той же функции как
// bytes.Buffer или strings.Builder, в аргументах fmt.Errorf. Секрет,
// доехавший до fmt.Errorf в обычной строковой переменной, сторожем не
// ловится вовсе.
//
